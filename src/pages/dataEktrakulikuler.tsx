...
import { IEktrakulikuler } from "../types/types";
import { toast } from 'react-toastify';

const DataEktrakulikuler: React.FC = () => {
    const [newsData, setNewsData] = useState<IEktrakulikuler[]>([]);
    ...
    const handleOpenEditModal = (data: IEktrakulikuler) => {
        setEditData(data);
        setOpenModal(true);
    };
    ...
    const handleSubmit = async () => {
        ...
        if (file) {
            formData.append("fotoEktra", file);
        }
    ...
    const handleDelete = async () => {
        ...
        try {
            await API.delete(`exktrakulikuler/${selectedId}`);
                        ...
                        {currentItems.map((data) => (
                            <tr key={data.id}>
                                <td className="py-1 px-4 border-b text-sm text-gray-700 border border-gray-400">
                                    {data.fotoEktra ? (
                                        <img
                                            src={`http://localhost:5001/uploads/${data.fotoEktra}`}
                                            alt="gambar"
                ...
                <Fade in={openModal}>
                    ...
                    >
                        <Typography variant="h6" component="h2" mb={2}>
                            {editData.id ? "Edit Ekstrakulikuler" : "Post Ekstrakulikuler"}
                        </Typography>
                        <TextField
                            label="Judul Ekstrakulikuler"
                            variant="outlined"
                            size="small"
                            fullWidth
                            margin="normal"
            ...
            >
                <Fade in={openConfirmModal}>
                    ...
                    >
                        ...
                        <Typography mb={2}>
                            Apakah Anda yakin ingin menghapus ekstrakulikuler ini?
                        </Typography>
...
export default DataEktrakulikuler;
